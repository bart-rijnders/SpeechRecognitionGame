...
document.addEventListener('DOMContentLoaded', (event) => {
    if (!('webkitSpeechRecognition' in window))
        ...
        textBar = new TextBar();
        gameState = new GameState(canvas, new VoiceRecognizer(), words);
        gameState.run();
    }  
})
...
function GameState(canvas, voiceRecognizer, words) {
    ...
    var spawn = function() {
        ...
        if (time>= lastSpawned + 500){
            ...
            var element = new TextElement(newWord, gameSpeed);
...
function VoiceRecognizer() {
    ...
    recognition.onresult = function(event) {
        for (var i = event.resultIndex; i < event.results.length; ++i) {
            words = event.results[i][0].transcript.split(' ');
        }
        words = words.map((x) => x.toLowerCase())
        textBar.text = words.toString();
        words.forEach(function(word) {
            var index = gameState.elements.findIndex((element) => element.text == word)
            if (index >= 0) {
